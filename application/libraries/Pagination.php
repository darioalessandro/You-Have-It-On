...
class CI_Pagination {
	...
	public function __construct($params = array())
	{
		...
		$this->first_link .= lang("first");
		$this->last_link = lang("last").$this->last_link;
	...
	function create_links()
	{
		...
		$end   = $num_pages; /*(($this->cur_page + $this->num_links) < $num_pages) ? $this->cur_page + $this->num_links : $num_pages;*/
		...
		{
			$this->base_url = ($this->javascript!='')? '': (rtrim($this->base_url, '/') .'/');
		}

		// And here we go...
		$output = '';
		
		$anchor_class = $this->anchor_class!=''? $this->anchor_class.'" ': '';
		$pre_anchor_class = $this->anchor_class!=''? 'class="': '';
		...
		{
			if($this->cur_page == 1){
				$output .= '<span class="previous">'.$this->prev_link.$this->cur_tag_close;
			}else{
				$i = $uri_page_number - $this->per_page;
	
				if ($i == 0 && $this->first_url != '')
				{
					$output .= $this->prev_tag_open.'<a '.$pre_anchor_class."previous ".$anchor_class.'href="'.$this->first_url.'">'.$this->prev_link.'</a>'.$this->prev_tag_close;
				}
				else
				{
			...
			for ($loop = $start -1; $loop <= $end; $loop++)
			{
				...
				if ($i >= 0)
				{
					...
					else
					{
						...
						if ($n == '' && $this->first_url != '')
						{
							$output .= $this->num_tag_open.'<a '.$pre_anchor_class.$anchor_class.'href="'.$this->first_url.'">'.$loop.'</a>'.$this->num_tag_close;
						}
						else
						{
