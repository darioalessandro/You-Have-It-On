...
if(is_array($items))
	...
	?>
		<div<?php echo ($id?' id="'.$id."-".$key.'"':'');?><?php echo ($class?' class="'.$class.$class_cols.'"':'');?> style="width:<?php echo $width_control-20; ?>px;<?php echo $height_items; ?>">
			<?php echo '<img src="'.(isset($item["images"][0]["file_name"])? $item["images"][0]["file_name"]: '').'" class="img img_friend" width="35" height="35">'; ?>
			
			<div class="user_content" style="width:<?php echo $width_control-60; ?>px;">
				<?php echo $item["name"]." ".$item["last_name"];?>
			</div>
			<div class="clear"></div>
			<?php
			if($view_actionbar){
