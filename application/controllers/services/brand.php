...
class Brand extends Services{
	...
	/**
	 * Servicio que agrega una marca. Las imagenes son opcionales, pero si hay, las sube
	...
	public function add(){
		...
		//if(isset($_POST['enviar'])){
			...
			if($this->status == true){
				...
				$data = $this->brand_model->saveBrand(self::$params, $images);
				if($data==false)
	...
	/**
	 * Servicio que modifica la informacion de una marca. Las imagenes son opcionales, pero si hay, las sube
	 * y las agrega a la bd
	 */
	public function edit(){
		...
		//if(isset($_POST['enviar'])){
			...
			if($this->status == true){
				...
				$this->load->model('brand_model');
				$data = $this->brand_model->updateBrand(self::$params, $images);
				if($data == -1)
	...
	/**
	 * Servicio que regresa un listado de marcas, paginadas y se pueden espesificar mas filtros
	 * para obtener marcas espesificas
	...
	public function query(){
		...
		if($this->status == true){
			...
			$data = $this->brand_model->getBrands(self::$params);
			if($data==false)
	...
	public function remove_image(){
		...
		if($this->status == true){
			...
			$this->load->model('brand_model');
			$data = $this->brand_model->removeImage(self::$params);
	...
	public function comment(){
		...
		if($this->status == true){
			...
			$this->load->model('brand_model');
			$data = $this->brand_model->comment(self::$params);
	...
	public function disable_comment(){
		...
		if($this->status == true){
			...
			$this->load->model('brand_model');
			$data = $this->brand_model->disable_comment(self::$params);
	...
	public function get_comments(){
		...
		if($this->status == true){
			$this->load->model('brand_model');
			$data = $this->brand_model->get_comments(self::$params);
	...
	/**
     * Valida los vectores website, phone_number y categorys.
     * @param ref $validate
     */
    private function validate_vectors(&$validate, $edit=false){
    	$bad = true;
    	
    	if(isset(self::$params['lat']) || isset(self::$params['lon'])){
    		if(!isset(self::$params['lat']))
    			return 'lat';
    		if(!isset(self::$params['lon']))
    			return 'lon';
    	}
    	...
    	if(isset(self::$params['brand_categories'])){
    		if(is_array(self::$params['brand_categories']) && count(self::$params['brand_categories']) > 0){
		    	foreach(self::$params['brand_categories'] as $key => $value){
		    		if(strlen($value) > 30 || strlen($value) < 1)
		    			$bad = 'brand_categories';
		    	}
    		}
    	}
