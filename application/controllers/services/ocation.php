...
class Ocation extends Services{
	private $status = true;
	...
	/**
	 * Servicio que agrega un ocation
	...
	public function add(){
		...
		if($this->status == true){
			...
			$data = $this->ocation_model->saveOcation(self::$params);
	...
	/**
	 * Servicio que regresa un listado de ocasiones, paginadas y se pueden espesificar mas filtros
	...
	public function query(){
		...
		if($this->status == true){
			$this->load->model('ocation_model');
			$data = $this->ocation_model->getOcation(self::$params);
			if($data==false)
